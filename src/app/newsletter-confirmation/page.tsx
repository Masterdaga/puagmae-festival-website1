...
export default function NewsletterConfirmation() {
  ...
  useEffect(() => {
    ...
    if (newsletterStatus === 'confirmed') {
      setStatus('confirmed');
      setMessage('Welcome to the PUAGMAE Festival community! Your newsletter subscription has been confirmed successfully.');
    } else if (newsletterStatus === 'unsubscribed') {
      setStatus('unsubscribed');
      setMessage('You have been successfully unsubscribed from our newsletter. We respect your decision.');
    } else {
      setStatus('error');
      setMessage('We encountered an issue with your confirmation link. Please try subscribing again.');
    }
  }, []);
        ...
        <div className="text-center">
          ...
          {status === 'confirmed' && (
            <div className="bg-black/40 backdrop-blur-sm p-8 rounded-xl border border-yellow-400/30 mb-8">
              <h3 className="text-yellow-400 text-xl font-semibold mb-4">🎉 What's Next?</h3>
              <ul className="text-yellow-200/90 text-left space-y-3">
                <li className="flex items-start">
                  <span className="text-yellow-400 mr-2">•</span>
                  <span>You'll receive exclusive festival updates and announcements</span>
                </li>
                <li className="flex items-start">
                  <span className="text-yellow-400 mr-2">•</span>
                  <span>Get early access to event registrations and special offers</span>
                </li>
                <li className="flex items-start">
                  <span className="text-yellow-400 mr-2">•</span>
                  <span>Stay connected with the vibrant PUAGMAE Festival community</span>
                </li>
                <li className="flex items-start">
                  <span className="text-yellow-400 mr-2">•</span>
                  <span>Be the first to know about performances, workshops, and activities</span>
                </li>
              </ul>
            </div>
          )}
          ...
          {status === 'unsubscribed' && (
            <div className="bg-black/40 backdrop-blur-sm p-8 rounded-xl border border-yellow-400/30 mb-8">
              <h3 className="text-yellow-400 text-xl font-semibold mb-4">We'll Miss You! 💛</h3>
              <p className="text-yellow-200/90 mb-4">
                Thank you for being part of our community. You can always resubscribe anytime by visiting our website and using the newsletter signup form in the footer.
              </p>
          ...
          {status === 'error' && (
            <div className="bg-black/40 backdrop-blur-sm p-8 rounded-xl border border-red-400/30 mb-8">
              <h3 className="text-red-400 text-xl font-semibold mb-4">What Happened?</h3>
              <ul className="text-yellow-200/90 text-left space-y-3">
                <li className="flex items-start">
                  <span className="text-red-400 mr-2">•</span>
                  <span>The confirmation link may have expired (links are valid for 24 hours)</span>
                </li>
                <li className="flex items-start">
                  <span className="text-red-400 mr-2">•</span>
                ...
                <li className="flex items-start">
                  <span className="text-red-400 mr-2">•</span>
                  <span>You can try subscribing again from our website homepage</span>
                </li>
              </ul>
            </div>
          )}
          ...
          <div className="mt-12 pt-8 border-t border-yellow-400/20">
            <p className="text-yellow-200/60 text-sm">
              © 2025 PUAGMAE Festival. Celebrating the African Golden 13th Month.
            </p>
          </div>
        </div>
      </div>
